...
class The_Billing_Software:
    def __init__(self, root):
        ...
        self.root.title(" Developed by Abhikriti Moti")
        ...
        # ------------------------------------------------Customer Frame------------------------------------------------
        Cust_Frame = LabelFrame(Main_Frame, text="Customer", font=("times new roman", 13, "bold"), bg="#EFEFEF",
                                fg="#0055ff")
        Cust_Frame.place(x=10, y=5, width=350, height=140)

        self.lbl_mob = Label(Cust_Frame, text="Mobile No.", font=("arial", 12, "bold"), bg="#EFEFEF")
        self.lbl_mob.grid(row=0, column=0, sticky=W, padx=10, pady=5)

        self.entry_mob = ttk.Entry(Cust_Frame, textvariable=self.c_mob, font=("times new roman", 10, "bold"), width=24)
        self.entry_mob.grid(row=0, column=1)

        vcmd = root.register(self.validate_mobile_input)
        self.entry_mob.config(validate="key", validatecommand=(vcmd, '%P'))

        self.hint_label = Label(root, text="Mobile number must be digits.", bg="black", fg="red",
        ...
        vcmd_name = root.register(self.validate_name_input)
        self.txtCustName.config(validate="key", validatecommand=(vcmd_name, '%P'))

        self.hint_label2 = Label(root, text="Name must be characters.", bg="black", fg="red",
                                 font=("Helvetica", 12, "bold"))
        self.hint_label2.pack_forget()  # Hide the hint label initially
        ...
        # -----------------------------------------------Product Frame ------------------------------------------------
        Product_Frame = LabelFrame(Main_Frame, text="Product", font=("times new roman", 13, "bold"), bg="#EFEFEF",
                                   fg="#0055ff")
        Product_Frame.place(x=370, y=5, width=620, height=140)

        # Category
        self.lblCategory = Label(Product_Frame, font=("arial", 12, "bold"), bg="#EFEFEF", text="Select Categories",
                                 bd=4)
        ...
        # SubCategory
        self.lblSubCategory = Label(Product_Frame, font=("arial", 12, "bold"), bg="#EFEFEF", text="Sub Categories",
                                    bd=4)
        ...
        self.ComboProduct = ttk.Combobox(Product_Frame, textvariable=self.product, font=("arial", 10, "bold"), width=24,
                                         state="readonly")
        ...
        def on_enter(e):
            btn_add.config(background='#002165', foreground="white", font=("arial", 12, "bold"))

        def on_leave(e):
            btn_add.config(background='#0055ff', foreground='white', font=("arial", 11, "bold"))

        btn_add = Button(MiddleFrame, text="Save", command=self.Add_db, cursor="hand2", font=("arial", 11, "bold"),
                         width=10, bg="#0055ff", fg="white")
        btn_add.grid(row=0, column=0, padx=28, pady=3)
        btn_add.bind('<Enter>', on_enter)
        btn_add.bind('<Leave>', on_leave)

        def on_enter(e):
            btn_upd.config(background='#002165', foreground="white", font=("arial", 12, "bold"))

        def on_leave(e):
            btn_upd.config(background='#0055ff', foreground='white', font=("arial", 11, "bold"))

        btn_upd = Button(MiddleFrame, text="Update", command=self.update_data, cursor="hand2",
                         font=("arial", 11, "bold"), width=10, bg="#0055ff", fg="white")
        btn_upd.grid(row=0, column=1, padx=28, pady=3)
        btn_upd.bind('<Enter>', on_enter)
        btn_upd.bind('<Leave>', on_leave)

        def on_enter(e):
            btn_del.config(background='#002165', foreground="white", font=("arial", 12, "bold"))

        def on_leave(e):
            btn_del.config(background='#0055ff', foreground='white', font=("arial", 11, "bold"))

        btn_del = Button(MiddleFrame, text="Delete", command=self.del_data, cursor="hand2", font=("arial", 11, "bold"),
                         width=10, bg="#0055ff", fg="white")
        ...
        # DbFrame
        db_frame = LabelFrame(MiddleFrame, relief=RIDGE, text=" Saved Bills ", font=("times new roman", 13, "bold"),
                              bg="#EFEFEF", fg="#0055ff")
        ...
        self.bold_centered_style = ttk.Style()
        self.bold_centered_style.configure("BoldCentered.Treeview.Heading", font=("Arial", 10, "bold"))
        self.bold_centered_style.configure("BoldCentered.Treeview", cell=["center"])
        self.bold_centered_style.configure("BoldCentered.Treeview", background="black",
                                           foreground="#EFEFEF", fieldbackground="#EFEFEF")

        self.db_table = ttk.Treeview(db_frame, column=("1", "2", "3", "4", "5"), xscrollcommand=scroll_x.set,
                                     yscrollcommand=scroll_y.set, style="BoldCentered.Treeview")
        ...
        self.db_table.heading("1", text="Invoice No.")
        ...
        for col in ("1", "2", "3", "4", "5"):
            self.db_table.heading(col, anchor="center")
        ...
        def on_enter(e):
            BtnSearch.config(background='#002165', foreground="white", font=("arial", 12, "bold"))

        def on_leave(e):
            BtnSearch.config(background='#0055ff', foreground='white', font=("arial", 11, "bold"))

        BtnSearch = Button(Search_Frame, command=self.find_bill, cursor="hand2", width=10, text="Search",
                           font=("arial", 11, "bold"), bg="#0055ff", fg="white")
        BtnSearch.grid(row=0, column=2)
        BtnSearch.bind('<Enter>', on_enter)
        BtnSearch.bind('<Leave>', on_leave)

        def on_enter(e):
            Btn_NewBill.config(background='#002165', foreground="white", font=("arial", 12, "bold"))

        def on_leave(e):
            Btn_NewBill.config(background='#0055ff', foreground='white', font=("arial", 11, "bold"))

        Btn_NewBill = Button(Search_Frame, command=self.NewBill, text="New Bill", bd=2, font=("arial", 11, "bold"),
                             bg="#0055ff", fg="white", width=10, cursor="hand2")
        Btn_NewBill.grid(row=0, column=3)
        Btn_NewBill.bind('<Enter>', on_enter)
        Btn_NewBill.bind('<Leave>', on_leave)

        # Bill Area
        RightLabelFrame = LabelFrame(Main_Frame, text="Bill Area", font=("times new roman", 13, "bold"), bg="#EFEFEF",
                                     fg="#0055ff")
        RightLabelFrame.place(x=1000, y=45, width=510, height=440)

        scroll_y = Scrollbar(RightLabelFrame, orient=VERTICAL)
        self.textarea = Text(RightLabelFrame, yscrollcommand=scroll_y.set, bg="white", fg="#00334d",
        ...
        # -----------------------------------------Bill Counter --------------------------------------------------------
        Bottom_Frame = LabelFrame(Main_Frame, text="Bill Counter", font=("times new roman", 13, "bold"), bg="#EFEFEF",
                                  fg="#0055ff")
        ...
        def on_enter(e):
            BtnAddToCart.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            BtnAddToCart.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        BtnAddToCart = Button(Btn_Frame, command=self.AddItem, cursor="hand2", width=15, height=2,
                              text="Add To Cart", font=("arial", 15, "bold"), bg="#0055ff", fg="white")
        BtnAddToCart.grid(row=0, column=0)
        BtnAddToCart.bind('<Enter>', on_enter)
        BtnAddToCart.bind('<Leave>', on_leave)

        def on_enter(e):
            Btngenerate.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            Btngenerate.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        Btngenerate = Button(Btn_Frame, command=self.gen_bill, cursor="hand2", width=12, height=2,
                             text="Generate Bill", font=("arial", 15, "bold"), bg="#0055ff", fg="white")
        Btngenerate.grid(row=0, column=1)
        Btngenerate.bind('<Enter>', on_enter)
        Btngenerate.bind('<Leave>', on_leave)

        def on_enter(e):
            BtnSave.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            BtnSave.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        BtnSave = Button(Btn_Frame, command=self.save_bill, cursor="hand2", width=12, height=2, text="Save Bill",
        ...
        def on_enter(e):
            BtnPrint.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            BtnPrint.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        BtnPrint = Button(Btn_Frame, command=self.iprint, cursor="hand2", width=12, height=2, text="Print",
                          font=("arial", 15, "bold"), bg="#0055ff", fg="white")
        BtnPrint.grid(row=0, column=3)
        BtnPrint.bind('<Enter>', on_enter)
        BtnPrint.bind('<Leave>', on_leave)

        def on_enter(e):
            BtnClear.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            BtnClear.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        BtnClear = Button(Btn_Frame, command=self.clear, cursor="hand2", width=12, height=2, text="Clear",
                          font=("arial", 15, "bold"), bg="#0055ff", fg="white")
        BtnClear.grid(row=0, column=4)
        BtnClear.bind('<Enter>', on_enter)
        BtnClear.bind('<Leave>', on_leave)

        def on_enter(e):
            BtnExit.config(background='#002165', foreground="white", font=("arial", 16, "bold"))

        def on_leave(e):
            BtnExit.config(background='#0055ff', foreground='white', font=("arial", 15, "bold"))

        BtnExit = Button(Btn_Frame, command=self.root.destroy, cursor="X_cursor", width=12, height=2, text="Exit",
                         font=("arial", 15, "bold"), bg="#0055ff", fg="white")
        BtnExit.grid(row=0, column=5)
        BtnExit.bind('<Enter>', on_enter)
        BtnExit.bind('<Leave>', on_leave)

        def on_enter(e):
            BtnInfo.config(background='#002165', foreground="white", font=("arial", 16, "bold"))
    ...
    def Add_db(self):
        ...
        if not self.c_mob.get() or not self.c_name.get() or not self.c_email.get():
            messagebox.showerror("Error", "Fill all the fields above to proceed with the modification.")
            return
        ...
        if self.bill_num.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', username="root", password="Le0n!d@s",
                                               database="management")
                mycursor = conn.cursor()
                mycursor.execute("insert into billling values(%s,%s,%s,%s,%s)", (
                    self.bill_num.get(), self.c_mob.get(), self.c_name.get(), self.c_email.get(), self.total.get()))
                conn.commit()
                self.fetch_data()
                conn.close()
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host='localhost', username="root", password="Le0n!d@s", database="management")
        mycursor = conn.cursor()
            ...
            try:
                ...
                if update > 0:
                    conn = mysql.connector.connect(host='localhost', username="root", password="Le0n!d@s",
                                                   database="management")
                    mycursor = conn.cursor()
                    mycursor.execute('update billling set c_mob=%s, c_name=%s, c_email=%s, total=%s where bill_num=%s',
                                     (self.c_mob.get(), self.c_name.get(), self.c_email.get(), self.total.get(),
                                      self.bill_num.get()))
    ...
    def gen_bill(self):
        if self.product.get() == "":
            ...
            self.textarea.insert(END, f"\n 0 Tax Amount:\t\t\t\t\t{self.tax_input.get()}")
    ...
    def validate_name_input(self, value):
        if value.isalpha() or value == "":
            self.hint_label2.pack_forget()
            return True
        else:
            self.hint_label2.pack()
            winsound.Beep(500, 300)
            return False
